...
class MeController extends Controller
{
    public function dashboard(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => [
            'prestations_today_count' => $todaysPrestations->count(),
    ...
    private function buildReport(Request $request): array
    {
        ...
        $clientsCount = $paid->pluck('client_id')->filter()->unique()->count()
            + $paid->whereNull('client_id')->count();
        $averageTicket = $paid->count() > 0 ? round($revenue / $paid->count(), 2) : 0.0;
        ...
        return [
            ...
            'commission_total' => round((float) $commissions->sum('amount'), 2),
            ...
            'details' => $prestations->sortByDesc('created_at')->values()->map(fn (Prestation $prestation) => [
