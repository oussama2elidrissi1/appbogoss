...
class PosV2WorkflowTest extends TestCase
{
    ...
    public function test_sale_lines_inside_a_v2_invoice_stay_separate_from_the_employee(): void
    {
        Sanctum::actingAs($this->superAdmin());
        ...
        $this->assertEquals(700, $historyEmployee['total']);
        $this->assertSame(1, $historyEmployee['performed_count']);
        $this->assertSame(0, $historyEmployee['sales_count']);
        ...
        $detail = $this->getJson("/api/pos-v2/invoices/{$invoice['id']}")
            ->assertOk()
            ->json('data');
        $saleLine = collect($detail['items'])->firstWhere('service_name', 'Vente gel');
        $this->assertNull($saleLine['employee_name']);
        ...
        $this->assertEquals(700, $ledgerRow['total']);
        $this->assertSame(1, $ledgerRow['performed_count']);
        $this->assertSame(0, $ledgerRow['sales_count']);
    }
