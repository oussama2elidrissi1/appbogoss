...
- **Affichage écran animé** (`/loyalty-qr/affichage`, bouton « Afficher à l'écran (animé) » sur la page QR Code) — version signalétique digitale plein écran pour tablette/écran au comptoir : logo BOGOSLAND animé (icône + lettres en cascade, halo pulsant), fond navy premium avec halos dorés dérivants et particules montantes, carte QR flottante avec coins dorés « respirants » et ligne de scan balayante, textes fixes dans la langue configurée (fr/ar/les deux, RTL correct). Sans sidebar/topbar, gardé derrière `loyalty.qr.manage`. Animation vérifiée en navigateur réel (deux captures à 2 s d'intervalle montrent le mouvement).
