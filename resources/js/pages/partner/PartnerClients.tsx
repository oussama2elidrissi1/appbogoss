    ...
    return (
        <motion.div variants={pageFade} initial="hidden" animate="show" className="space-y-6">
            ...
            {isPending ? (
                ...
                <EmptyState
                    icon={Users}
                    title={filter === 'archived' ? 'Aucun client archivé' : 'Aucun client'}
                    description={
                        filter === 'archived'
                            ? 'Les clients archivés apparaîtront ici.'
                            : 'Les clients que vous apportez apparaîtront ici.'
                    }
                />
            ) : (
                <div className="space-y-2">
                        ...
                        <Card key={client.id} className={cn('flex flex-wrap items-center gap-4 p-4', client.archived_at && 'opacity-60')}>
                            ...
                            <div className="flex shrink-0 items-center gap-1.5">
                                ...
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    aria-label={client.archived_at ? 'Désarchiver' : 'Archiver'}
            ...
            <ConfirmDialog
                ...
                title={archiveTarget?.archived_at ? 'Désarchiver ce client ?' : 'Archiver ce client ?'}
                description={
                    archiveTarget?.archived_at
                        ? `${archiveTarget?.name} réapparaîtra dans votre liste active.`
                        : `${archiveTarget?.name} n'apparaîtra plus dans votre liste active — son historique est conservé.`
                }
                confirmLabel={archiveTarget?.archived_at ? 'Désarchiver' : 'Archiver'}
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-sm">
                <DialogHeader>
                    ...
                    <DialogDescription>Il sera ajouté à votre portefeuille, visible uniquement par vous et BOGOSLAND.</DialogDescription>
                </DialogHeader>
                <div className="space-y-3">
                    <div className="space-y-1.5">
                        <Label htmlFor="new-client-name-list">Nom</Label>
                        <Input id="new-client-name-list" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nom du client" />
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="new-client-phone-list">Téléphone</Label>
                        <Input id="new-client-phone-list" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="06 00 00 00 00" />
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="new-client-email-list">Email (facultatif)</Label>
