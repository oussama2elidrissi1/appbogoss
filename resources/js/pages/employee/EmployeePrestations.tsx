...
export default function EmployeePrestations() {
    ...
    return (
        <EmployeePageShell>
            <div>
                <h2 className="text-2xl font-semibold">Mes prestations</h2>
                <p className="text-sm text-white/50">Suivi de vos services, montants et commissions.</p>
            </div>
            ...
            <EmployeePanel className="p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                    ...
                    >
                        ...
                        <option value="draft">A venir</option>
                        <option value="in_progress">En cours</option>
                        <option value="services_done">Terminee</option>
                        <option value="pending_payment">Caisse</option>
                        <option value="paid">Payee</option>
                        <option value="cancelled">Annulee</option>
                    </select>
                    <div className="relative min-w-0 flex-1">
                        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" />
                        <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Recherche client" className="border-white/[0.08] bg-white/[0.04] pl-9 text-white" />
                    </div>
                </div>
            </EmployeePanel>

            <EmployeePanel className="p-4">
                <EmployeePanelTitle title="Nouvelle prestation" icon={CheckCircle2} />
                <div className="pt-4">
                    <NewPrestationPanel />
                </div>
            </EmployeePanel>
                    ...
                    <table className="w-full min-w-[900px] text-sm">
                        ...
                        <tbody>
                            {isPending ? (
                                ...
                                <tr><td colSpan={9} className="px-4 py-10 text-center text-white/50">Aucune prestation sur cette periode.</td></tr>
                            ) : data.map((row) => (
                                <PrestationRow
                                    key={row.id}
                                    row={row}
...
function PrestationRow({ row, onComplete, onSend, completing, sending }: {
    ...
    return (
        <tr className="border-t border-white/[0.06] text-white/76">
            ...
            <td className="px-4 py-3 text-right">{row.duration_minutes} min</td>
            ...
            <td className="px-4 py-3 text-right"><Badge variant="outline">{statusLabel(row.status)}</Badge></td>
