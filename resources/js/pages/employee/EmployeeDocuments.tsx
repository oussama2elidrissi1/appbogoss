...
export default function EmployeeDocuments() {
    ...
    return (
        <EmployeePageShell>
            <div>
                <h2 className="text-2xl font-semibold">Mes documents</h2>
                <p className="text-sm text-white/50">Documents autorises pour votre compte.</p>
            </div>
            <EmployeePanel>
                <EmployeePanelTitle title="Documents" icon={FileText} />
                <div className="p-4">
                    {!data || data.documents.length === 0 ? (
                        <p className="rounded-md border border-white/[0.07] bg-white/[0.035] px-4 py-12 text-center text-white/50">
                            {data?.empty_state ?? 'Aucun document disponible.'}
                        </p>
